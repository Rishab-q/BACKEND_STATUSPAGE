...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5174","https://front-status-1.onrender.com"],
...
async def update_service(
    ...
    await update_status(service, update.status, db,user=user)
  ...
  try:
    ...
    for svc in event.affected_services:
        ...
        if event.type==Eventtype.INCIDENT:
            ...
            await update_status(service, svc.new_status, db,user=user)
...
async def update_status(service:Service, status:ServiceStatus, db:Session,user=Depends(get_current_user)):
...
from db import get_db  # Assuming get_db is in your db.py

async def async_sched(event: Event, db: Session, user):
    affected_services = db.query(Affected_services_model).filter_by(event_id=event.id).all()

    for svc in affected_services:
        if svc.service.status == ServiceStatus.OPERATIONAL:
            await update_status(svc.service, ServiceStatus.MAINTENANCE, db, user=user)
        elif svc.service.status == ServiceStatus.MAINTENANCE:
            await update_status(svc.service, ServiceStatus.OPERATIONAL, db, user=user)

    db.commit()
def sched(event: Event, db: Session = Depends(get_db), user=Depends(get_current_user)):
    asyncio.run(async_sched(event, db, user))
...
def schedule_event(event: Event, db: Session = Depends(get_db), user=Depends(get_current_user)):
    scheduler.add_job(
        ...
        args=[event, db, user],  # Pass event, db session, and user
